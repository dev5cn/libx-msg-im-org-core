...
#ifndef XMSGIMORGCFG_H_
#define XMSGIMORGCFG_H_
...
{
public:
	shared_ptr<XmsgImOrgCfgPb> cfgPb; 
	SptrCgt cgt; 
	SptrCgt hlr; 
public:
	static shared_ptr<XmsgImOrgCfg> instance(); 
	static shared_ptr<XmsgImOrgCfg> load(const char* path); 
	static void setCfg(shared_ptr<XmsgImOrgCfg> cfg); 
	shared_ptr<XscTcpCfg> xscServerCfg(); 
	string toString();
	XmsgImOrgCfg();
	virtual ~XmsgImOrgCfg();
private:
	static shared_ptr<XmsgImOrgCfg> cfg; 
