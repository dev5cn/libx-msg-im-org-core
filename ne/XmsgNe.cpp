...
void XmsgNe::estab()
{
	...
	req->set_neg(X_MSG_IM_ORG);
	req->set_cgt(XmsgImOrgCfg::instance()->cfgPb->cgt());
	req->set_salt(Crypto::gen0aAkey256());
	req->set_sign(Crypto::sha256ToHexStrLowerCase(XmsgImOrgCfg::instance()->cfgPb->cgt() + req->salt() + this->pwd));
	req->set_alg(this->alg);
	if (this->neg == X_MSG_IM_HLR)
		XmsgMisc::insertKv(req->mutable_ext(), "sub-x-msg-client-disc-event", "enable"); 
